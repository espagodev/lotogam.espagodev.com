...
@section('content')
    <div class="row pt-2 pb-2 no-print">
        ...
        request()->session()->get('user.bancaBloqueo') == 0)
                    <a href="{{ route('pos.create') }}" class="btn btn-primary waves-effect waves-primary"><i
                            class="fa fa-ticket mr-1"></i> Crear Ticket</a>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="modal fade ticket_modal no-print" tabindex="-1" role="dialog" aria-labelledby="gridSystemModalLabel">
    </div>

    <div class="modal fade pagar_premio_modal no-print" tabindex="-1" role="dialog" aria-labelledby="gridSystemModalLabel">
    </div>
